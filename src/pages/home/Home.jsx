...
import { ReactComponent as ToTop } from "../../images/icon/ToTop.svg";
...
const ToTopBtn = styled(({ show, right, ...rest }) => <div {...rest} />)`
  display: flex;
  ...
  width: 70px;
  height: 70px;
  box-shadow: 0px 6px 10px rgba(62, 62, 62, 0.15);
...
function Home() {
  ...
  const [showToTopBtn, setShowToTopBtn] = useState();
  const refEle = useRef();

  useEffect(() => {
    const scrollCB = () =>
      setShowToTopBtn(
        refEle.current.clientHeight / 2 < refEle.current.scrollTop
      );
    refEle.current.addEventListener("scroll", scrollCB);
  }, []);

  return (
    <ScrolledContainer ref={refEle}>
      <Container>
        <FullHeightContent
        ...
        <ToTopBtn
          show={showToTopBtn}
          right={isSmScreen ? "24" : "70"}
          onClick={() => {
            refEle.current.scrollTo({ top: 0, behavior: "smooth" });
